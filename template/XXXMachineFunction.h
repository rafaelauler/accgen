...
public:
  ...
  void printGlobalValues(raw_ostream &O, Mangler* Mang, const std::string &tgt)
  const {
    for (unsigned i = 0, e = GlobalValues.size(); i != e; ++i) {
      // Globals generated by JumpTable lowering must be processed in order to
      // include this function number (supplied in tgt). 
      std::string result(Mang->getValueName(GlobalValues[i].gv));
      std::string::size_type i;  
      std::string marker("_MFUNCNUM_");
